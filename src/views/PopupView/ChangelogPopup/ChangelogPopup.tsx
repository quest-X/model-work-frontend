...
const CHANGELOG_DATA: ChangelogEntry[] = [
    {
        ...
        changes: [
            ...
            { zh: '【功能】导出 UI 重设计：矩形 / 多边形模式下导出弹窗改为两组选项——格式（LabelMe 标注包 / YOLO 训练包）+ 模式（简单：仅标签文件 / 完整：标签 + 图像），四种组合对应四种 zip 前缀（labelme_simple / labelme_full / yolo_simple / yolo_train）', en: '[Feature] Export UI redesign: rect/polygon export popup now offers two selector groups — format (LabelMe annotation pack / YOLO training pack) and mode (simple: labels only / complete: labels + images), yielding four zip variants prefixed labelme_simple / labelme_full / yolo_simple / yolo_train' },
            { zh: '【功能】YOLO 分割导出：新增 YOLOPackExporter，自动检测标注类型——有多边形时输出 YOLO seg 格式（归一化多边形点串），仅矩形时输出 YOLO det 格式（cx cy w h）；矩形在分割模式下自动转为四点多边形；完整包含 images/{train,val,test}/ + labels/{train,val,test}/ + data.yaml + labels.txt，80/10/10 划分', en: '[Feature] YOLO segmentation export: new YOLOPackExporter auto-detects type — polygons produce YOLO seg format (normalized point string), rects-only produce YOLO det format; rects convert to 4-point polygons in seg mode; complete pack includes images/labels split dirs, data.yaml, labels.txt with 80/10/10 split' },
            { zh: '【功能】LabelMe 完整包扁平结构 + 一键导入：labelme_full_*.zip 内 JSON 与图像同级放置（符合 LabelMe 标准格式）；拖入该 zip 自动解包图像并就地导入标注，无需提前加载视频；labelme_simple_*.zip 提示先加载对应图像', en: '[Feature] LabelMe full pack flat structure + one-drop import: labelme_full_*.zip stores JSON and images at the same level (standard LabelMe format); dropping the zip auto-unpacks images and imports annotations without pre-loading video; labelme_simple_*.zip prompts to load images first' },
            { zh: '【优化】所有导出 zip 内新增与文件名同名的根文件夹，解压后直接得到单一文件夹而非散落文件', en: '[Improvement] All export zips now wrap files in a root subfolder named after the zip, so extraction yields a single folder instead of loose files' },
        ]
    },
    {
        version: '2.2.0',
