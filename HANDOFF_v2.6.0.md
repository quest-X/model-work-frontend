# v2.6.0 WebCodecs 重构 — 实施计划与归档

## 状态: 暂停 (2026-05-08)

**结论:** 在用户实际数据规模上没有体现出明显优于 v2.5.4 的优势,暂存分支不合并到主线。

| 视频规模 | v2.5.4 | v2.6.0 | 备注 |
|---|---|---|---|
| ≤ 1080p · 短视频 | ✅ 流畅 | ✅ 流畅 | 平手 |
| 1440p · 短视频 | ✅ 流畅 (含三层缓存修复) | ✅ 流畅 | 平手 |
| 1440p · 中等长度 | ✅ 不崩,远端 scrub ~500ms | ✅ 接近 | 内存少一些,体感无感 |
| **9.3 GB / 1440p / 10min** | ✅ 不崩 (v2.5.4 已修) | ⚠️ Fetch timeout | **v2.5.4 反而更稳** |

**为什么 9.3GB 上 v2.6.0 反而不行**: 后端在后台拼命跑 FFmpeg 把 mp4 拆成 15094 个 JPEG (推理用),
跟 mp4 stream Range 请求抢 CPU + 磁盘 I/O,大文件 Range 超时。架构层面要解决这个,
需让 backend 知道 WebCodecs 模式下推迟 / 跳过 JPEG 拆帧 — 工作量不小。

**保留分支的价值**:
- 完整可工作的 WebCodecs decode pipeline (mp4 demux + GPU 解码 + LRU)
- 类型扩展 (HTMLImageElement | VideoFrame)、helper、lifecycle 管理都跑通
- 未来如果有 4K / 8K / 真正长视频需求时,可以在此基础上继续投入

**不合并的原因**:
- 用户实际数据 ≤ 1440p 短/中视频,v2.5.4 已够用
- 9.3GB 的边界场景需要后端协同改造,投入产出比一般
- v2.5.4 在所有当前场景下表现≥v2.6.0,稳定性更好
...
- 2026-05-07：Phase 1-3 service 层落地（commit `b9c7940`）
  - **Phase 1 后端 Range** ✅ `/video-stream/{session_id}` 路由 + FastAPI FileResponse
  - **Phase 2 mp4 demux** ✅ `services/VideoDemuxer.ts`，mp4box@0.5.4（避开 TS 5.0 类型语法）
  - **Phase 3 decoder** ✅ `services/WebCodecsFramePlayer.ts`，GPU 硬解 + LRU 50 帧
  - **依赖**：mp4box@0.5.4、@types/dom-webcodecs

## Phase 3 wire-in（下一步）

**核心难点**：把 `WebCodecsFramePlayer.getFrame(idx) → VideoFrame` 接进 `FramePlayer.drawFrame`，但 `EditorModel.videoFrameImage` / `EditorActions.setActiveImage` / `VideoPrimaryRenderEngine` 当前都假设 `HTMLImageElement`。需要做的类型扩展：

```ts
// EditorModel.ts
public static videoFrameImage: HTMLImageElement | VideoFrame | null;
public static image: HTMLImageElement | VideoFrame;

// EditorActions.ts
public static setActiveImage(image: HTMLImageElement | VideoFrame) { ... }

// VideoEditor.tsx:254-255
// 把 .naturalWidth/.naturalHeight 改成 helper:
function getW(img: HTMLImageElement | VideoFrame) {
    return 'naturalWidth' in img ? img.naturalWidth : img.displayWidth;
}
```

`drawImage(img, ...)` 同时支持两种类型（都是 `CanvasImageSource`），渲染层基本不动。

### 具体步骤
1. 更新 `EditorModel` / `EditorActions` 的类型签名
2. 加 `frameSourceUtil.ts`，提供 `getWidth/getHeight/close` helpers
3. `FramePlayer.tsx` 加 `webcodecsUrl?: string` prop
4. 当 `webcodecsUrl` 存在：
   - mount: `VideoDemuxer.create()` + `WebCodecsFramePlayer.create()`
   - drawFrame: `await player.getFrame(idx) → drawImage`
   - 跳过 `maintainAvailableFrames`/`evictOldFrames`（不需要 JPEG 滑动窗）
   - unmount: `player.close()`
5. `VideoEditor.tsx` 在 sessionId 存在 + WebCodecs 可用时，构造 `webcodecsUrl = ${API_URL}/video-stream/${sessionId}` 传给 FramePlayer

### Phase 3 wire-in 风险评估
- **高**：动 `EditorModel.image` 类型 → 全项目可能数十处需要 type narrow / helper
- **中**：`VideoFrame` 必须显式 `close()`，遗漏会内存泄漏
- **中**：playback 时 frame ahead pre-decode 策略与现有滑动窗机制不一样，需要重写

### 建议
- Phase 3 wire-in 单开 PR，配上 v2.5.x JPEG fallback 开关，可随时切回
- 先在 1080p 中等长度视频测试，再上 1440p / 4K
- 性能基准录屏 v2.5.4 vs v2.6.0 对比
